...
export default function KioskPage() {
  ...
  // ----------------------------------------------------
  // 1. 왼쪽 화면: 실시간 현황 불러오기 로직 (useEffect 내부로 안전하게 이동)
  // ----------------------------------------------------
  useEffect(() => {
    let isMounted = true;
    ...
    const fetchActiveMembers = async () => {
      ...
      if (!error && data && isMounted) {
        // 💡 해결 1: @ts-expect-error 뒤에 3글자 이상의 설명을 추가했습니다.
        // @ts-expect-error: Supabase 조인(Join) 데이터의 복잡한 타입 추론 생략
  ...
  }, []); // 💡 해결 2: 의존성 배열이 깔끔하게 비워져 렌더링 최적화 완료!
      ...
      <section className="w-[45%] h-full bg-[#fdfbf7] border-r-8 border-[#2c1e16] flex flex-col shadow-2xl relative z-10">
        <header className="p-6 bg-white border-b-2 border-stone-200 flex justify-between items-end">
          <div>
            ...
            {/* 💡 해결 3: 작은따옴표를 HTML 표준 기호(&apos;)로 변경했습니다. */}
        ...
        <div className="relative z-10 w-full max-w-md">
          {isSuccess ? (
            ...
            <div className="bg-[#3a291f] p-8 rounded-[40px] shadow-2xl">
              ...
              {/* 💡 해결 4: 키패드 버튼의 active:translate-y-[6px]를 표준인 active:translate-y-1.5로 수정했습니다. */}
